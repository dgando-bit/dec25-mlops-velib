...
```
### Activation du mode développement dans Airflow
-	Pendant vos tests, il vous sera peut-être utile d'activer le `mode_dev`. 
  - Depuis l'interface Airflow: Admin → Variables → ajouter `dev_mode = true`.
- Pour forcer l'entrainement du modèle, ajouter la variable `force_training`. 
  - Depuis l'interface Airflow: Admin → Variables → ajouter `dev_mode = true`.
