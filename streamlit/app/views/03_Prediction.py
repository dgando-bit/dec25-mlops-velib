...
def _render_svg_zoomable(svg_path: str, height: int = 480, key: str = "diag-pred"):
...
with right_col:
    ...
    if predict_btn:
        ...
        # État initial — niveaux d'alerte + schéma du cycle d'inférence
        st.markdown("""
<div style="background:#f0f8eb;border-radius:12px;padding:1.5rem;text-align:center;margin-bottom:1rem;">
...
""")
        ...
        st.divider()
        st.markdown("#### Cycle complet d'une requête d'inférence")
        st.caption(
            "POST /predict → nginx (rate limit 10 r/s) → API FastAPI → "
            "cache LRU → predict_with_confidence (24 features) → "
            "reconstruction taux = trend_avg + résidu → alert_level."
        )
        _render_svg_zoomable("assets/diagrams/04_inference.svg", height=480, key="diag-inference")
